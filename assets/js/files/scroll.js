...
function stickyAnchors() {
	...
	const header = document.querySelector(".header");
	const anchors = document.querySelector(".anchors__body");

	if (header && anchors) {
		const headerRect = header.getBoundingClientRect();
		const anchorsRect = anchors.getBoundingClientRect();

		if (headerRect.bottom >= anchorsRect.top && !isAnchor) {
			anchors.classList.add("_sticky");
			anchors.style.top = `${headerRect.bottom}px`;

			anchorStart = currentScroll;
			isAnchor = true;

			if (anchorStart === null) {
				anchorStart = headerRect.bottom;
			}
		}

		if (anchorStart > currentScroll && isAnchor) {
